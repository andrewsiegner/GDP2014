...
GLuint ObjectColourUniformLocation = 0;	// The "slot" ID of our colour variable in the shader
...
/*  const Vertex VERTICES[8] =
  {
    { { -.5f, -.5f,  .5f, 1 }, { 0, 0, 1, 1 } },   // A single vertex
    { { -.5f,  .5f,  .5f, 1 }, { 1, 0, 0, 1 } },
    { {  .5f,  .5f,  .5f, 1 }, { 0, 1, 0, 1 } },
    { {  .5f, -.5f,  .5f, 1 }, { 1, 1, 0, 1 } },
    { { -.5f, -.5f, -.5f, 1 }, { 1, 1, 1, 1 } },
...
void DrawCube(void)
{
	...
	for ( int index = 0; index != static_cast<int>( vec_pRenderedObjects.size() ); index++ )
	{
		...
		glUniform4f( ObjectColourUniformLocation, vec_pRenderedObjects[index]->colour.x, 
